...
const BlacklistPage = () => {
  ...
  const loggedInUserId = searchParams.get('userId'); // Get the logged-in user's ID
  ...
  const fetchBlacklist = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/user/blacklist`);
  ...
  useEffect(() => {
    ...
    // 2. Filter by Role
    if (roleFilter !== "All") {
      // FIX: Apply capitalizeFirstLetter to user.role to match the dropdown value
      result = result.filter(user => capitalizeFirstLetter(user.role) === roleFilter);
    }

    setFilteredUsers(result);
  }, [searchTerm, roleFilter, blacklistedUsers, loggedInUserId]);

  // --- Action Handlers ---

  // 1. Trigger Confirmation Dialog
  const handleRemoveFromBlacklistClick = (userId, email) => {
    setDialog({
        isOpen: true,
        type: 'confirm',
        title: 'Confirm Action',
        message: 'Are you sure you want to remove this user from the blacklist?',
        onConfirm: () => executeRemoveFromBlacklist(userId, email)
    });
  };

  // 2. Execute API Call
  const executeRemoveFromBlacklist = async (userId, email) => {
    ...
    <div style={{ display: 'flex' }}>
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <button 
            onClick={() => router.push(`/adminUsers?userId=${loggedInUserId}`)}
            style={{
              padding: '12px 24px',
              background: 'linear-gradient(135deg, #1e40af 0%, #3b82f6 100%)',
              color: 'white',
        ...
        <div style={{ 
          ...
          <input 
            type="text" 
            placeholder="Search Blacklisted User by ID or Name..." 
            value={searchTerm}
                ...
                {filteredUsers.length === 0 ? (
                    ...
                    >
                        ...
                        <td style={{ padding: '16px 20px', textAlign: 'center' }}>
                            <button 
                              onClick={() => handleRemoveFromBlacklistClick(user.user_id, user.email)}
                              style={{ 
                                padding: '8px 16px', 
                                background: 'linear-gradient(135deg, #1e40af 0%, #3b82f6 100%)', 
                                color: 'white', 
